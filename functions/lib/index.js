...
async function findDeviceByClaimCode(claimCode) {
 ...
 */
export const deviceRegister = onRequest(FUNCTION_CONFIG, async (req, res) => {
    if (req.method !== "POST") {
 ...
 */
export const deviceClaim = onRequest(FUNCTION_CONFIG, async (req, res) => {
    if (req.method !== "POST") {
        sendErrorResponse(res, 405, "Method not allowed");
        return;
    }
 ...
 */
export const devicePairingStatus = onRequest(FUNCTION_CONFIG, async (req, res) => {
    if (req.method !== "GET") {
        sendErrorResponse(res, 405, "Method not allowed");
        return;
    }
    try {
        ...
        if (paired && data?.deviceTokenOnce && data?.tokenDelivered === false) {
            const token = data.deviceTokenOnce;
            await deviceDoc.ref.set({
                tokenDelivered: true,
 ...
 */
export const deviceCurrentFocusSession = onRequest(FUNCTION_CONFIG, async (req, res) => {
    if (req.method !== "GET") {
        sendErrorResponse(res, 405, "Method not allowed");
        return;
    }
    try {
        ...
        const fsData = fsDoc.exists ? fsDoc.data() : {};
        okJson(res, {
 ...
 */
export const deviceSessionSummary = onRequest(FUNCTION_CONFIG, async (req, res) => {
    if (req.method !== "POST") {
        sendErrorResponse(res, 405, "Method not allowed");
        return;
    }
    try {
        ...
        const focusSession = focusSessionDoc.exists ? focusSessionDoc.data() : null;
        const device = deviceDoc.data();
...
export const focusStart = onRequest(FUNCTION_CONFIG, async (req, res) => {
    ...
    try {
        ...
        await deviceRef.set({ activeFocusSessionId: focusSessionId, updatedAt: new Date() }, { merge: true });
